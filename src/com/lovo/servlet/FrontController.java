...
public class FrontController extends HttpServlet {
	...
	private void injectProperties(Action action, HttpServletRequest req) throws Exception {
		...
		while(paramNamesEnum.hasMoreElements()) {
			...
			if(fieldType != null) {
				...
				if(fieldType.isArray()) {	// 如果属性是数组类型
					...
					System.out.println("from FrontControl:"+req.getParameter(paramName));
	   ...
	   private  void injectService(Object obj) {
	        Class<?> cls = obj.getClass();//Class.forName(clas);
			Field[] fields = cls.getDeclaredFields();
			//ConcurrentHashMap a= new ConcurrentHashMap();
			for (Field field : fields)
                if (field.isAnnotationPresent(InjectService.class) == true) {
                    //System.out.println(field.getType());
                    field.setAccessible(true);
                    try {
                        //field.set(obj, ServiceFactory.get(field.getType()));
                        field.set(obj, ServiceFactory.factory(field.getType()));
